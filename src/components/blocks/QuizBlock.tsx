...
const QuizBlock: React.FC<BlockProps> = ({ id, content, onUpdate }) => {
  const [quizData, setQuizData] = useState<QuizData>(
    content ? JSON.parse(content) : { title: '', questions: [] }
  );

  const handleTitleChange = (title: string) => {
    const newData = { ...quizData, title };
    setQuizData(newData);
  ...
  const handleCorrectAnswerChange = (questionIndex: number, correctAnswer: number) => {
    const newQuestions = [...quizData.questions];
    newQuestions[questionIndex] = { ...newQuestions[questionIndex], correctAnswer };
    const newData = { ...quizData, questions: newQuestions };
    setQuizData(newData);
    onUpdate(id, JSON.stringify(newData));
  };
  ...
  const addQuestion = () => {
    const newQuestions = [...quizData.questions, { question: '', options: [''], correctAnswer: 0 }];
    const newData = { ...quizData, questions: newQuestions };
    setQuizData(newData);
    onUpdate(id, JSON.stringify(newData));
  };
